...
glm::vec3 bunny_position(0.0f, 0.0f, 0.0f);
...
const int n_assets = 64; // Počet inicializovaných objektů
...
static void init_glfw(void)
{
	...
	globals.window = glfwCreateWindow(800, 600, "Bacha zajic", NULL, NULL);
	if (!globals.window)
	{
...
int main()
{
	...
	if (glewInit() != GLEW_OK) {
		std::cerr << "Chyba při inicializaci GLEW" << std::endl;
		return -1;
	}
	...
	texture_id[1] = gen_tex("resources/textures/background.jpg");
	texture_id[2] = gen_tex("resources/textures/box.png");
	texture_id[3] = gen_tex("resources/textures/ball.jpg");
	texture_id[4] = gen_tex("resources/textures/bat.png");
	texture_id[5] = gen_tex("resources/textures/sky.jpg");
	texture_id[6] = gen_tex("resources/textures/wall.jpg");
	...
	while (!glfwWindowShouldClose(globals.window)) {
...

		{
			...
			glm::vec3 direction_to_player = player_position * 0.45f - bunny_position + target_offset;
			direction_to_player = glm::normalize(direction_to_player);
			glm::vec3 distance = direction_to_player * ball_speed;
			bunny_position = bunny_position + distance;
			//bal
			m_m = glm::translate(m_m, bunny_position);
			m_m = glm::rotate(m_m, glm::radians(720.0f * ((float)glfwGetTime()/4)), glm::vec3(0.0f, 0.20f, 0.0f));
			glUniformMatrix4fv(glGetUniformLocation(prog_h, "uM_m"), 1, GL_FALSE, glm::value_ptr(m_m));
			...
			//std::cout << "ball " << bunny_position.x << " " << bunny_position.z << " player " << player_position.x * 0.5f << " " << player_position.z * 0.5f << std::endl;
			check_rabbit_collision(); //collision check
...
void check_rabbit_collision() {
	...
	if (p_p.x > ball_coords.min_x && p_p.x < ball_coords.max_x && p_p.z > ball_coords.min_z && p_p.z < ball_coords.max_z) {
		if (!oofing) {
			engine->play2D("resources/sounds/bite.wav");
			oofing = true;
		}
	}
	else oofing = false;
...
void setup_objects() {
	...
	// Textured Grass
	int grass_index = 0;
	assets[grass_index].type = asset_type_texture;
	assets[grass_index].tex_vertex_array.push_back({ {-10.0f, -0.9f, -10.0f}, glm::vec2(-5.0f, -5.0f), up });
	assets[grass_index].tex_vertex_array.push_back({ { -10.0f, -0.9f, 10.0f}, glm::vec2(-5.0f, 5.0f), up });
	assets[grass_index].tex_vertex_array.push_back({ { 10.0f, -0.9f, 10.0f}, glm::vec2(5.0f, 5.0f), up });
	assets[grass_index].tex_vertex_array.push_back({ {10.0f, -0.9f, 10.0f}, glm::vec2(5.0f, 5.0f), up });
	assets[grass_index].tex_vertex_array.push_back({ {10.0f, -0.9f, -10.0f}, glm::vec2(5.0f, -5.0f), up });
	assets[grass_index].tex_vertex_array.push_back({ {-10.0f, -0.9f, -10.0f}, glm::vec2(-5.0f, -5.0f), up });
	assets[grass_index].indices_array = { 0, 1, 2, 3, 4, 5 };
	PrepareVAO(grass_index);

	// Textured Forest - Front
	int wall_front_index = 14;
	assets[wall_front_index].type = asset_type_texture;
	assets[wall_front_index].tex_vertex_array.push_back({ { -11.0f, -1.0f, 11.0f}, glm::vec2(-1.0f, -1.0f), up });
	assets[wall_front_index].tex_vertex_array.push_back({ { -11.0f, 9.0f, 11.0f}, glm::vec2(-1.0f, 1.0f), up });
	assets[wall_front_index].tex_vertex_array.push_back({ { 11.0f, 9.0f, 11.0f }, glm::vec2(1.0f, 1.0f), up });
	assets[wall_front_index].tex_vertex_array.push_back({ { 11.0f, 9.0f, 11.0f}, glm::vec2(1.0f, 1.0f), up });
	assets[wall_front_index].tex_vertex_array.push_back({ { 11.0f, -1.0f, 11.0f}, glm::vec2(1.0f, -1.0f), up });
	assets[wall_front_index].tex_vertex_array.push_back({ { -11.0f, -1.0f, 11.0f}, glm::vec2(-1.0f, -1.0f), up });
	assets[wall_front_index].indices_array = { 0, 1, 2, 3, 4, 5 };
	PrepareVAO(wall_front_index);

	// Textured Forest - Right
	int wall_right_index = 15;
	assets[wall_right_index].type = asset_type_texture;
	assets[wall_right_index].tex_vertex_array.push_back({ { 11.0f, -1.0f, 11.0f}, glm::vec2(-1.0f, -1.0f), up });
	assets[wall_right_index].tex_vertex_array.push_back({ { 11.0f, 9.0f, 11.0f}, glm::vec2(-1.0f, 1.0f), up });
	assets[wall_right_index].tex_vertex_array.push_back({ { 11.0f, 9.0f, -11.0f }, glm::vec2(1.0f, 1.0f), up });
	assets[wall_right_index].tex_vertex_array.push_back({ { 11.0f, 9.0f, -11.0f}, glm::vec2(1.0f, 1.0f), up });
	assets[wall_right_index].tex_vertex_array.push_back({ { 11.0f, -1.0f, -11.0f}, glm::vec2(1.0f, -1.0f), up });
	assets[wall_right_index].tex_vertex_array.push_back({ { 11.0f, -1.0f, 11.0f}, glm::vec2(-1.0f, -1.0f), up });
	assets[wall_right_index].indices_array = { 0, 1, 2, 3, 4, 5 };
	PrepareVAO(wall_right_index);

	// Textured Forest - Back
	int wall_back_index = 16;
	assets[wall_back_index].type = asset_type_texture;
	assets[wall_back_index].tex_vertex_array.push_back({ { 11.0f, -1.0f, -11.0f}, glm::vec2(-1.0f, -1.0f), up });
	assets[wall_back_index].tex_vertex_array.push_back({ { 11.0f, 9.0f, -11.0f}, glm::vec2(-1.0f, 1.0f), up });
	assets[wall_back_index].tex_vertex_array.push_back({ { -11.0f, 9.0f, -11.0f }, glm::vec2(1.0f, 1.0f), up });
	assets[wall_back_index].tex_vertex_array.push_back({ { -11.0f, 9.0f, -11.0f}, glm::vec2(1.0f, 1.0f), up });
	assets[wall_back_index].tex_vertex_array.push_back({ { -11.0f, -1.0f, -11.0f}, glm::vec2(1.0f, -1.0f), up });
	assets[wall_back_index].tex_vertex_array.push_back({ { 11.0f, -1.0f, -11.0f}, glm::vec2(-1.0f, -1.0f), up });
	assets[wall_back_index].indices_array = { 0, 1, 2, 3, 4, 5 };
	PrepareVAO(wall_back_index);

	// Textured Forest - Left
	int wall_left_index = 17;
	assets[wall_left_index].type = asset_type_texture;
	assets[wall_left_index].tex_vertex_array.push_back({ { -11.0f, -1.0f, -11.0f}, glm::vec2(-1.0f, -1.0f), up });
	assets[wall_left_index].tex_vertex_array.push_back({ { -11.0f, 9.0f, -11.0f}, glm::vec2(-1.0f, 1.0f), up });
	assets[wall_left_index].tex_vertex_array.push_back({ { -11.0f, 9.0f, 11.0f }, glm::vec2(1.0f, 1.0f), up });
	assets[wall_left_index].tex_vertex_array.push_back({ { -11.0f, 9.0f, 11.0f}, glm::vec2(1.0f, 1.0f), up });
	assets[wall_left_index].tex_vertex_array.push_back({ { -11.0f, -1.0f, 11.0f}, glm::vec2(1.0f, -1.0f), up });
	assets[wall_left_index].tex_vertex_array.push_back({ { -11.0f, -1.0f, -11.0f}, glm::vec2(-1.0f, -1.0f), up });
	assets[wall_left_index].indices_array = { 0, 1, 2, 3, 4, 5 };
	PrepareVAO(wall_left_index);

    // === Skybox ===
    int skybox_index = 19;
    assets[skybox_index].type = asset_type_texture;
    float skybox_size = 11.0f;
	assets[skybox_index].tex_vertex_array.push_back({ { -skybox_size, -skybox_size,  skybox_size}, glm::vec2(0.0f, 1.0f), up });
    assets[skybox_index].tex_vertex_array.push_back({ {  skybox_size, -skybox_size,  skybox_size}, glm::vec2(1.0f, 1.0f), up });
    assets[skybox_index].tex_vertex_array.push_back({ {  skybox_size, -skybox_size,  skybox_size}, glm::vec2(1.0f, 1.0f), up });
    assets[skybox_index].tex_vertex_array.push_back({ {  skybox_size, -skybox_size, -skybox_size}, glm::vec2(1.0f, 0.0f), up });
    assets[skybox_index].tex_vertex_array.push_back({ { -skybox_size, -skybox_size, -skybox_size}, glm::vec2(0.0f, 0.0f), up });
	assets[skybox_index].indices_array = { 0, 1, 2, 3, 4, 5 };
    PrepareVAO(skybox_index);
	...
	assets[index].type = asset_type_texture;
	assets[index].color = { 0.7, 0.7, 0.0 };
	assets[index].scale = { 2.2, 2.2, 2.2 };
	assets[index].coord = { 0, 0, 0 };
	loadOBJ("resources/obj/cube.obj", assets[index].vertex_array, assets[index].indices_array, assets[index].color, assets[index].scale, assets[index].coord);
	for (int i = 0; i < (int)(assets[index].vertex_array.size()); i++)
	{
	...
	index = 10;
	assets[index].type = asset_type_texture;
	assets[index].scale = { 0.2, 0.2, 0.2 };
	assets[index].coord = { 0, 0, 0 };
	...
	assets[index].scale = { 0.7, 0.7, 0.7 };
	assets[index].coord = { 0, 0, 5 };
	loadOBJ("resources/obj/bat.obj", assets[index].vertex_array, assets[index].indices_array, assets[index].color, assets[index].scale, assets[index].coord);
	for (int i = 0; i < (int)(assets[index].vertex_array.size()); i++)
	{
		assets[index].tex_vertex_array.push_back({ assets[index].vertex_array[i].position, assets[index].vertex_array[i].texCoor, assets[index].vertex_array[i].normal });
	}
	...
	// Transparentish teapot
	index = 18;
	assets[index].type = asset_type_color;
	assets[index].color = { 0.1, 1.0, 0.1 };
	assets[index].scale = { 0.1, 0.1, 0.1 };
	assets[index].coord = { 0.0, 0.0, 5 };
...
void draw_textured(glm::mat4 m_m, glm::mat4 v_m, glm::mat4 projectionMatrix) {
    ...
    // Define the mapping of assets to texture IDs
    struct AssetTextureMapping {
        int asset_index;
        int texture_index;
    };

    AssetTextureMapping mappings[] = {
        {0, 0}, 
		{14, 1},{15, 1}, {16, 1}, {17, 1},
		{18, 3},{19, 5},{9, 2},{10, 3},{11, 4}, {13,2},
		{1, 6},{2, 6}, {3, 6},{4, 6},{5, 6},{6, 6}, {7, 6},{8, 6}
	};

    for (const auto& mapping : mappings) {
        int asset_index = mapping.asset_index;
        int texture_index = mapping.texture_index;

        if (asset_index >= 0 && asset_index < sizeof(assets)/sizeof(assets[0]) &&
            texture_index >= 0 && texture_index < sizeof(texture_id)/sizeof(texture_id[0])) {

            glBindVertexArray(assets[asset_index].VAO);
            glBindTexture(GL_TEXTURE_2D, texture_id[texture_index]);

            // Check for errors before drawing
            CHECK_GL_ERROR();

            glDrawElements(GL_TRIANGLES, assets[asset_index].indices_array.size(), GL_UNSIGNED_INT, 0);
...
void draw_transparent(glm::mat4 m_m, glm::mat4 v_m, glm::mat4 projectionMatrix) {
	...
	glBindVertexArray(assets[18].VAO);
	glDrawElements(GL_TRIANGLES, assets[18].indices_array.size(), GL_UNSIGNED_INT, 0);
	glDisable(GL_BLEND);
	glEnable(GL_CULL_FACE);
	glDepthMask(GL_TRUE);
	glUseProgram(prog_h);
